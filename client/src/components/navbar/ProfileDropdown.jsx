...
const ProfileDropdown = ({ user, isEducator, isAdmin, signOut }) => {
  ...
  const menuItems = isAdmin
    ...
    : [
        ...
        { label: 'My Quizzes', icon: '❓', href: '/quizzes' },
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 rounded-xl p-1 pr-3 hover:bg-slate-100 dark:hover:bg-dk-surface-2 transition-colors group"
      ...
      >
        ...
        <div className="hidden xl:block text-left">
          <p className="text-xs font-semibold text-slate-800 dark:text-dk-text leading-tight max-w-[100px] truncate">
            {user?.firstName || user?.fullName || 'User'}
          </p>
          <p className="text-[10px] text-slate-400 dark:text-dk-muted">{roleLabel}</p>
        </div>
        <svg
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 z-50 w-64 rounded-2xl border border-slate-200 dark:border-dk-border bg-white/95 dark:bg-dk-surface-2 backdrop-blur-xl shadow-2xl shadow-black/10 dark:shadow-black/40 overflow-hidden"
          >
            {/* User info header */}
            <div className="px-4 py-3 border-b border-slate-100 dark:border-dk-border">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-800 dark:text-dk-text truncate">
                    {user?.fullName || 'User'}
                  </p>
                  <p className="text-xs text-slate-400 dark:text-dk-muted truncate">
  ...
  {menuItems.map((item, index) => (
    <Link
      ...
      className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-600 dark:text-dk-muted hover:bg-slate-50 dark:hover:bg-dk-surface hover:text-slate-900 dark:hover:text-dk-text transition-colors"
    >
      ...
      {item.label}
            ...
            {/* Sign out */}
            <div className="p-1.5 border-t border-slate-100 dark:border-dk-border">
              <button
