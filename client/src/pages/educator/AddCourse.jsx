    ...
    <div className='h-screen overflow-scroll flex flex-col items-start md:p-8 p-4 pt-8 bg-gray-50'>
      <form onSubmit={handleSubmit} className='w-full max-w-3xl space-y-5'>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input type='text' value={courseTitle} onChange={(e) => setCourseTitle(e.target.value)} placeholder='Type Here' className='outline-none border border-gray-400 rounded-md p-2' required />
        </div>

        {/* Quill Editor */}
        <div className='flex flex-col gap-1'>
          <label className='text-gray-600 font-medium mb-2'>Course Description</label>
        ...
        <div className='flex gap-4'>
          <div className='flex-1'>
            ...
            <input type='number' value={coursePrice} onChange={(e) => setCoursePrice(e.target.value)} className='w-full border border-gray-400 rounded-md p-2' />
          </div>
          <div className='flex-1'>
            <label className='text-gray-600 font-medium'>Discount (%)</label>
            <input type='number' value={courseDiscount} onChange={(e) => setCourseDiscount(e.target.value)} className='w-full border border-gray-400 rounded-md p-2' />
          </div>
        </div>
          ...
          {chapters.map((chapter, index) => (
            <div key={chapter.chapterId} className="bg-white border rounded-lg overflow-hidden">
              <div className='flex justify-between items-center p-4 bg-gray-100'>
                ...
                <img src={assets.cross_icon} className='cursor-pointer w-3' onClick={() => handleChapter('remove', chapter.chapterId)} />
              </div>
        ...
        <button type='submit' className='bg-black text-white px-8 py-2.5 rounded font-bold'>ADD COURSE</button>
      </form>
      ...
      {showPopup && (
        <div className='fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50'>
          <div className='bg-white p-6 rounded-lg w-full max-w-md relative'>
            ...
            <img onClick={() => setShowPopup(false)} src={assets.cross_icon} className='absolute top-4 right-4 cursor-pointer w-4' />
          </div>
        </div>
      )}
    </div>
