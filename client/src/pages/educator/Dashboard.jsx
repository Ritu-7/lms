    ...
    <div className='min-h-screen bg-slate-50 dark:bg-dk-base p-4 md:p-8 space-y-8'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6'>
        {stats.map((stat, idx) => (
          <motion.div 
            ...
            className='bg-white dark:bg-dk-surface rounded-2xl p-6 border border-slate-200 dark:border-dk-border shadow-sm flex items-center gap-4'
          >
            <img src={stat.icon} alt={stat.label} className='w-12 h-12' />
            <div>
              <p className={`text-2xl font-bold font-space-grotesk ${stat.color} dark:text-dk-text`}>{stat.value}</p>
              <p className='text-sm text-slate-500 dark:text-dk-text-2 font-medium'>{stat.label}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {[
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: idx * 0.1 }}
            className='bg-white dark:bg-dk-surface rounded-2xl p-6 border border-slate-200 dark:border-dk-border shadow-sm'
          >
            <p className='text-sm font-medium text-slate-500 dark:text-dk-text-2'>{item.label}</p>
            <p className='mt-2 text-3xl font-bold font-space-grotesk text-slate-900 dark:text-dk-text'>{item.value}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className='bg-white dark:bg-dk-surface rounded-2xl border border-slate-200 dark:border-dk-border shadow-sm overflow-hidden'>
        <div className='p-6 border-b border-slate-200 dark:border-dk-border'>
          <h2 className='text-xl font-bold font-space-grotesk text-slate-900 dark:text-dk-text'>Latest Enrollments</h2>
        </div>
        <div className='overflow-x-auto'>
          <table className='w-full text-left text-sm'>
            <thead className='text-slate-500 bg-slate-50 dark:bg-dk-surface'>
              <tr>
            ...
            <tbody className='divide-y divide-slate-100 dark:divide-dk-border'>
              {(dashboardData.enrolledStudentsData || dashboardData.enrolledStudents)?.length > 0 ? (
                (dashboardData.enrolledStudentsData || dashboardData.enrolledStudents).slice(0, 5).map((item, index) => {
                  ...
                  return (
                    <tr key={index} className='hover:bg-slate-50 dark:hover:bg-dk-surface/50 transition-colors'>
                      ...
                      <td className='px-6 py-4 flex items-center gap-3'>
                        ...
                        />
                        <span className='font-semibold text-slate-900 dark:text-dk-text capitalize'>{displayName}</span>
                      </td>
                      <td className='px-6 py-4 text-slate-600 dark:text-dk-text-2'>{item.courseTitle || "Untitled Course"}</td>
                    </tr>
                  );
                })
              ) : (
