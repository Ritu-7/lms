  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 lg:p-12 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search activities..." className="pl-9 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
          </div>
      ...
      {loading ? (
        ...
        <div className="flex items-center gap-3 text-rose-600 bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-900/40 rounded-2xl px-4 py-3"><AlertTriangle size={16} />{error}</div>
      ) : !hasData ? (
        <div className="rounded-3xl border border-dashed border-slate-300 dark:border-white/10 bg-white dark:bg-slate-900 px-6 py-14 text-center text-slate-500 dark:text-slate-400">
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex items-center gap-4">
            ...
            <div className="xl:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">Track your AI usage over time.</p>
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm p-6">
            <h3 className="text-lg font-bold font-space-grotesk text-slate-900 dark:text-white mb-4">Recent AI Activity</h3>
            {analytics.recent?.length ? (
              <div className="grid gap-3">
                {analytics.recent.map((item) => (
                  <div key={item.id} className="flex items-center justify-between rounded-xl border border-slate-100 dark:border-white/10 px-4 py-3">
                    <div>
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{item.title || item.feature}</p>
